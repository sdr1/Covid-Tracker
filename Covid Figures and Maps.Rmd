...
#   ggtitle(c(paste("CDC Data As Of", as.Date(max(CDC_w_trendline$Date))))) +
#   theme(
...
CDC_cases <- CDC_w_trendline %>%
  ...
  summarise(total_cases_by_day = sum(total_cases_by_day),
            total_deaths_by_day = sum(total_deaths_by_day)) %>%
  ggplot(aes(x = Date, y = total_cases_by_day, fill = Region)) +
  geom_area() +
  theme_minimal() +
  scale_fill_brewer() +
  scale_y_continuous(label = scales::comma) +
  xlab("Date") + ylab("Total Cases") + 
  ggtitle(c(paste("Total Cases: CDC Data As Of", as.Date(max(CDC_w_trendline$Date))))) +
  theme(
    plot.title = element_text(size=14, face="bold",hjust = 0.5),
    axis.title.x = element_text(size=14, face="bold"),
    axis.title.y = element_text(size=14, face="bold"),
...
CDC_deaths <- CDC_w_trendline %>%
  ...
  summarise(total_cases_by_day = sum(total_cases_by_day),
            total_deaths_by_day = sum(total_deaths_by_day)) %>%
  ggplot(aes(x = Date, y = total_deaths_by_day, fill = Region)) +
  geom_area() +
  theme_minimal() +
  scale_fill_brewer() +
  scale_y_continuous(label = scales::comma) +
...
plot_grid(CDC_cases, CDC_deaths, ncol=2)
#CDC

```

## Covid Cases in Somerville, MA in the Last Two Weeks
```{r Somerville, echo = F, warning = F,cache = T, message = F, fig.width = 13, fig.height = 8.5}
local <- fread("https://somerville-dashboardcovid.trial.opendatasoft.com/explore/dataset/daily-c19-case-counts/download/?format=csv&timezone=America/New_York&lang=en&use_labels_for_header=true&csv_separator=%2C")
...
```{r deaths_by_county,echo = F, warning = F,cache = T, message = F, fig.width = 13, fig.height = 8.5}
# Overall level on the most recent day
# Deaths by county
usmap::plot_usmap("counties", labels = F, include = state.abb,
                  data = CDC_total %>% 
  ...
  theme(legend.position = "right") + 
  theme_void() + xlab("") + ylab("") 
ggsave("deaths_by_county.pdf", width = 11, height = 8.5)
```


## Top 5 Counties: Today and The First of Each Month
```{r top counties,echo = F, warning = F,cache = T, message = F, fig.width = 13, fig.height = 8.5}
...
```{r,echo = F, warning = F,cache = T, message = F, fig.width = 15, fig.height = 8.5}
# New cases by state by day, log scale, with week long trend line
# use log scale otherwhise NY and MA overwhelm other states
CDC_w_trendline %>% #filter(State == "OK") %>%
  ggplot(aes(x = Date, y = total_cases_by_day)) +
...
```{r hospitalizations,echo = F, warning = F,cache = T, message = F, fig.width = 13, fig.height = 8.5}
covid_tracking_data %>% 
...
```{r week-to-week,echo = F, warning = F,cache = T, message = F, fig.width = 13, fig.height = 8.5}
...
```{r New Northeast, echo = F, warning = F,cache = T, message = F, fig.width = 11, fig.height = 8.5}
...
```{r,, echo = F, warning = F,cache = T, message = F, fig.width = 11, fig.height = 8.5}
# Cases over time
...
```{r deaths by state,  echo = F, warning = F,cache = T, message = F, fig.width = 11, fig.height = 8.5}
plot_usmap(data = CDC_total %>% 
             dplyr::rename(state = State) %>%
             filter(Date > "2020-02-01" & 
                      grepl("-01$", Date)) %>%
...
```{r ratio, echo = F, warning = F,cache = T, message = F, fig.width = 11, fig.height = 8.5}
covid_tracking_data %>% 
  filter(State %in% state.abb) %>%
  mutate( Day = lubridate::yday(Date),
          Week =  lubridate::week(Date),
...
```{r icu, echo = F, warning = F,cache = T, message = F, fig.width = 11, fig.height = 8.5}
covid_tracking_data %>% 
  filter(State %in% state.abb) %>%
  mutate( Day = lubridate::yday(Date),
          Week =  lubridate::week(Date)) %>%
...
```{r vent, echo = F, warning = F,cache = T, message = F, fig.width = 11, fig.height = 8.5}
covid_tracking_data %>% 
  filter(State %in% state.abb) %>%
  mutate( Day = lubridate::yday(Date),
          Week =  lubridate::week(Date)) %>%
...
```{r differences, echo = F, warning = F,cache = T, message = F, fig.width = 11, fig.height = 8.5}
