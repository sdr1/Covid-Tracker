...
```{r world map, eval=F, echo=F}
# Fix map, scale is bad
...
small_translate <- small_translate %>% select(country.name.en, iso3c)

names(small_translate)[1] <- "Country"

global_map <- global %>%
  left_join(small_translate, by = "Country") %>%
  rename(country = Country) %>% filter(Date == max(Date))

visitedMap <- joinCountryData2Map(global_map , 
                                  joinCode = "NAME",
                                  nameJoinColumn = "country")

qtiles <- quantile(global_map$Cumulative_cases)

mapParams <- mapCountryData( visitedMap, 
                            nameColumnToPlot="Cumulative_cases",
                            oceanCol = "azure2",
                            catMethod=c(0,500,5000,50000,500000,5000000),
