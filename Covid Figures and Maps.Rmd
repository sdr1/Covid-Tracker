...
state_with_most <- CDC_w_trendline %>%   
  filter(Date == max(Date, na.rm = T)) %>% 
  filter(new_cases == max(new_cases)) %>%
  dplyr::select(State, new_cases,Date)
...
* `r Somerville_MA %>% filter(Date %in% (max(Date)-14):max(Date)) %>% summarize(new_cases = sum(new_cases, na.rm = T)) %>% pull()` cases in the last two weeks  
  + Out of `r formatC(Somerville_MA %>% filter(Date == max(Date)) %>% select(Positive) %>% pull(), format="f", big.mark=",", digits=0)` cases overall   
...
Where my grandparents live, in Collier County, FL, as of `r format(max(CDC_total$Date), '%B %d, %Y')` there are:

* `r formatC(CDC_total %>% filter(countyFIPS == 12021) %>% mutate(new_cases = Cases - dplyr::lag(Cases)) %>% filter(Date == max(Date)) %>% pull(), format="f", big.mark=",", digits=0)` new cases
* `r formatC(CDC_total %>% filter(countyFIPS == 12021 & Date %in% (max(Date)-15):max(Date)) %>% mutate(new_cases = Cases - dplyr::lag(Cases))  %>% filter(Date %in% (max(Date)-14):max(Date))  %>% summarize(new_cases = sum(new_cases, na.rm = T)) %>% pull(), format="f", big.mark=",", digits=0)` cases in the last two weeks
...
```{r World Map, echo = F, warning = F,cache = F, message = F, fig.width = 13, fig.height = 8.5}
...
small_translate <- countrycode::codelist

global_map <- global %>%
  rename(ecb = Country_code) %>%
  left_join(small_translate, by = "ecb") %>%
  rename(country = Country) %>% filter(Date == max(Date)) %>%
  rename(iso_a3 = iso3c)

World <- World %>% 
  left_join(global_map, by = "iso_a3") %>%
  mutate(Cases_per_capita = Cumulative_cases / pop_est * 1000)


#qtm(World, fill = "Cumulative_cases", fill.pallete = "RdYlGn") + tmap_style("natural")
## tmap style set to "classic"
## other available styles are: "white", "gray", "natural", "cobalt", "col_blind", "albatross", "beaver", "bw", "watercolor"
#https://www.rdocumentation.org/packages/tmap/versions/3.0/topics/tm_layout
raw_cases <- tm_shape(World) +
  tm_polygons(col = "Cumulative_cases", 
              #style = "order",
              legend.hist = F,
              palette = "YlOrRd",
...
# to make maps better
cases_per_capita <- tm_shape(World) +
  tm_polygons(col = "Cases_per_capita", 
              style = "log10_pretty",
              legend.hist = F,
              palette = "YlOrRd",
              title="Cases per Captia",
              main.title.position = "center",
              main.title.size = 14,
              main.title.fontface = "bold") +
  tm_layout(legend.outside = T) +
...
knitr::kable(Latest_Near_Me %>% dplyr::select(Date,`County Name`, Cases, Deaths, `New Cases`, `New Deaths`)) %>%
...
knitr::kable(Top5Counties %>% dplyr::select(Date, abbr,`County Name`, Cases, Deaths) %>% dplyr::rename(State = abbr)) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
...
state_election_2016 <- election_2016 %>% group_by(state_po, candidate) %>% 
  ...
  rename(State = state_po) %>% dplyr::select(State, Winner) %>%
  ungroup()
...
difference_in_counts <- difference_in_counts %>% 
  dplyr::select(Date, CDC_total_cases_by_day, CT_total_cases_by_day, 
